...
def init_kv_cache(max_seq_len, d_model):
    ...
    kv_cache = dict()
    kv_cache['length'] = 0
    kv_cache['K'] = np.zeros((max_seq_len, d_model), dtype = np.float32)
    kv_cache['V'] = np.zeros((max_seq_len, d_model), dtype = np.float32)
    return kv_cache
...
def append_kv(cache, k_new, v_new):
    ...
    shape = (cache['length'], k_new.shape[0])
    cache['K'][shape[0]:shape[0] + shape[1]] = k_new
    cache['V'][shape[0]:shape[0] + shape[1]] = v_new
    cache['length'] += shape[1]
    return cache
...
def causal_attention(q, k, v, is_causal=True):
    ...
    storage = np.matmul(q, np.transpose(k))
    storage /= np.sqrt(q.shape[-1])

    if is_causal:
        mask = np.triu(np.ones((storage.shape[0], storage.shape[1]), dtype=bool), k=1 + (storage.shape[1] - storage.shape[0]))
        storage[mask] = -np.inf
    storage = stable_softmax((storage))
    return storage @ v
...
def model_prefill(token_ids, params):
    ...
    cache = init_kv_cache(params['max_seq_len'], params['embedding'].shape[1])
    cache = append_kv(cache, K, V)

    A = causal_attention(Q, K, V, is_causal=True)
    A = linear_projection(A, params['Wo'])
    logits = linear_projection(A[-1], params['W_out'])
